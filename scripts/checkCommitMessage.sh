...
CONVENTIONAL_REGEX='^(feat|feature|fix|docs|style|refactor|perf|test|build|ci|chore|revert|i18n|bump)(\([a-zA-Z0-9._/-]+\))?(!)?: .+'
...
validate_subject() {
  ...
  if ! is_valid_subject "$subject"; then
    ...
    echo "  feat(sync): add recovery flow"
    echo "  fix: prevent null pointer in import"
    echo "  chore!: drop deprecated API"
    echo "  bump: version 1.3.5 -> 1.3.6"
    return 1
  fi
}
